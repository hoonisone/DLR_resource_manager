...
class ModelConfig(AutoSavingModel):
    name: str
    age: int
    a_path: Path = Field(default_factory=lambda: Path("aaa.bbb.json"))
...
# def test_path_auto_rel_abs_convert():


#     file_path = Path("aaa/bbb/ccc.json")
#     memo = MemoFactory().make_file_memo(file_path)

#     x = ModelConfig(_memo=memo, name="resnet", age=50, a_path="ddd.json")
    
#     # 속성 반환시 path는 모두 절대 경로화
#     assert x.a_path == Path("aaa/bbb/ccc/ddd.json")
